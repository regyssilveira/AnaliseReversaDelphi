...
begin
  ...
  try
    ...
    Html.Add('let selected=null,selectedPath=target,mode=nodes.size>250?"explore":"all",expanded=new Set([target]),currentPos=new Map(),traceResult=null,traceStep=0,cutEdges=new Set(),remainingRoute=null,zoom=1,baseWidth=0,baseHeight=0,dragState=null;function show(text){detail.textContent=text}');
    ...
    Html.Add('function render(){const filter=document.getElementById("dprOnly").checked,section=document.getElementById("sectionFilter").value;const term=document.getElementById("search").value.toLowerCase(),highlighted=traceNodes(),traceEdges=new Set(traceResult?.edges||[]);if(mode==="dprRoutes")for(const e of dprEdges){highlighted.add(e.from);highlighted.add(e.to)}let visible,routeEdges=[];');
