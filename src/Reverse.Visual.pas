...
begin
  ...
  try
    ...
    Html.Add('<button id="route">Ver caminho da seleção até o DPR</button>');
    ...
    Html.Add('let selected=null,selectedPath=target,mode="all",currentPos=new Map();function show(text){detail.textContent=text}');
    Html.Add('function route(start){const queue=[start],previous=new Map([[start,null]]);for(let i=0;i<queue.length;i++){const x=queue[i];if(x===program)break;for(const e of out.get(x)||[])if(!previous.has(e.to)){previous.set(e.to,e);queue.push(e.to)}}if(!previous.has(program))return [];const path=[];for(let x=program;x!==start;){const e=previous.get(x);path.unshift(e);x=e.from}return path}');
    ...
    Html.Add('function selectNode(path){selected=path;selectedPath=path;if(!toDpr.has(path))document.getElementById("dprOnly").checked=false;const name=nodes.get(path).name;show(`${name}\n${path}\nUsada por ${(out.get(path)||[]).length} declaração(ões)\nUsa ${(incoming.get(path)||[]).length} declaração(ões)${toDpr.has(path)?"\nTem caminho até o DPR":""}`);render();center(path)}');
    ...
    Html.Add('else if(mode==="route"){routeEdges=route(selectedPath);visible=new Set([selectedPath]);for(const e of routeEdges){visible.add(e.from);visible.add(e.to)}}');
    Html.Add('else visible=filter?new Set([...toDpr].filter(x=>nodes.has(x))):new Set(nodes.keys());');
    Html.Add('const root=mode==="all"?target:selectedPath;const levels=new Map([[root,0]]),queue=[root];for(let i=0;i<queue.length;i++)for(const e of out.get(queue[i])||[])if(visible.has(e.to)&&!levels.has(e.to)){levels.set(e.to,levels.get(queue[i])+1);queue.push(e.to)}');
    ...
    Html.Add('document.getElementById("status").textContent=mode==="route"?selectedPath===program?"A seleção já é o DPR.":routeEdges.length?`Caminho mais curto até o DPR: ${routeEdges.length} ligação(ões).`:`Sem caminho da seleção até o DPR.`:mode==="neighbors"?`Ligações diretas da unit selecionada: ${Math.max(0,visible.size-1)} vizinho(s).`:`Grafo completo: ${visible.size} arquivo(s).`;');
    Html.Add('}');
    ...
    Html.Add('document.getElementById("neighbors").addEventListener("click",()=>{mode="neighbors";render();center(selectedPath)});document.getElementById("route").addEventListener("click",()=>{mode="route";render();center(selectedPath)});');
    Html.Add('document.getElementById("reset").addEventListener("click",()=>{selected=null;selectedPath=target;mode="all";document.getElementById("search").value="";document.getElementById("dprOnly").checked=false;updateMatches();show("");render();center(target)});render();');
