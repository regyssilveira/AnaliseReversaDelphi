...
begin
  ...
  try
    ...
    Html.Add('<div id="detail"></div><div id="trace"></div></aside><main id="viewport"><svg id="graph"></svg></main></div>');
    Html.Add('<script>');
    ...
    Html.Add('let selected=null,selectedPath=target,mode="all",currentPos=new Map(),traceResult=null;function show(text){detail.textContent=text}');
    Html.Add('function inProject(path){return !!projectRoot&&path.toLowerCase().startsWith(projectRoot.toLowerCase())}');
    ...
    Html.Add('let step=0;for(const e of traceResult.edges){const b=document.createElement("button");b.textContent=`${++step}. ${e.used} → ${e.consumer} | ${e.section}:${e.line} | ${e.source}`;b.addEventListener("click",()=>{selected=e;show(`${e.used} → ${e.consumer}\nDeclaração: ${e.source}:${e.line}\nArquivo usado: ${e.from}\nConsumidor: ${e.to}`);render();center(e.to)});panel.appendChild(b)}');
    ...
    Html.Add('function selectNode(path){selected=path;selectedPath=path;traceResult=trace(path);if(!toDpr.has(path))document.getElementById("dprOnly").checked=false;const name=nodes.get(path).name;show(`${name}\n${path}\nUsada por ${(out.get(path)||[]).length} declaração(ões)\nUsa ${(incoming.get(path)||[]).length} declaração(ões)${toDpr.has(path)?"\nTem caminho até o DPR":""}`);showTrace();render();center(path)}');
    ...
    Html.Add('document.getElementById("reset").addEventListener("click",()=>{selected=null;selectedPath=target;traceResult=null;mode="all";document.getElementById("search").value="";document.getElementById("dprOnly").checked=false;updateMatches();show("");document.getElementById("trace").replaceChildren();render();center(target)});render();');
