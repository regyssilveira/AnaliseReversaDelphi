...
begin
  ...
  try
    ...
    Html.Add('input,button,select{box-sizing:border-box;width:100%;padding:9px;margin:5px 0;background:#0d1117;color:#e6edf3;border:1px solid #30363d;border-radius:6px}button{cursor:pointer}button:hover{border-color:#58a6ff}.switch{display:grid;grid-template-columns:1fr 1fr;gap:5px}.switch button.active{border-color:#58a6ff;background:#152f4a}#folders{margin-top:12px}#folders button{text-align:left;font-size:12px;white-space:normal}#detail{white-space:pre-wrap;word-break:break-word;line-height:1.5;border-top:1px solid #30363d;margin-top:12px;padding-top:12px}');
    ...
    Html.Add('<button id="fit">Ajustar grafo</button><p id="status"></p><div id="folders"></div><div id="detail">Selecione uma pasta, unit ou ligação.</div>');
    ...
    Html.Add('const panel=document.getElementById("folders");for(const [folder,group]of [...folders].sort((a,b)=>a[0].localeCompare(b[0]))){const b=document.createElement("button");b.textContent=`${folder} (${group.length})`;b.onclick=()=>{mode="folders";document.getElementById("search").value=folder;setMode("folders")};panel.appendChild(b)}');
