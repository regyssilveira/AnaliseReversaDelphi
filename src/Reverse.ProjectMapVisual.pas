...
begin
  ...
  try
    ...
    Html.Add('<div id="navigation"><button id="folderBack" disabled>← Voltar para o nível anterior</button><div class="switch"><button id="folderMode" class="active">Pastas</button><button id="unitMode">Units</button></div></div>');
    ...
    Html.Add('function openPhysicalFolder(folder){saveNavigation();activeFolders=new Set([folder]);const group=folders.get(folder)||[];document.getElementById("search").value="";groupDetail(folder,group);setMode("units")}function resetFolders(){activeFolders=null;folderHistory=[];document.getElementById("search").value="";setMode("folders")}');
