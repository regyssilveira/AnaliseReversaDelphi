...
begin
  ...
  try
    ...
    Html.Add('const panel=document.getElementById("folders");for(const [folder,group]of [...folders].sort((a,b)=>a[0].localeCompare(b[0]))){const details=document.createElement("details"),summary=document.createElement("summary");summary.textContent=`${folder} (${group.length})`;summary.onclick=()=>openFolder(folder);details.appendChild(summary);for(const f of group.sort((a,b)=>a.name.localeCompare(b.name))){const b=document.createElement("button");b.textContent=f.name;b.title=f.path;b.onclick=()=>{document.getElementById("search").value=f.path;setMode("units");document.getElementById("detail").textContent=`${f.name}\n${f.path}\n\nPasta: ${f.folder}\nOrigem: ${f.origin}`};details.appendChild(b)}panel.appendChild(details)}');
