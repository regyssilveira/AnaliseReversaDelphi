...
begin
  ...
  try
    ...
    Html.Add('const folderDepthSelect=document.getElementById("folderDepth"),relativeFolders=[...folders.keys()].filter(x=>!["(raiz do projeto)","(desconhecido)"].includes(x)&&!(/^[a-z]:[\\/]/i.test(x)||x.startsWith("\\\\"))),maxFolderDepth=Math.max(1,...relativeFolders.map(x=>x.split(/[\\/]+/).length));for(let depth=2;depth<=maxFolderDepth;depth++){const option=document.createElement("option");option.value=String(depth);option.textContent=`${depth} níveis`;folderDepthSelect.appendChild(option)}const fullDepth=document.createElement("option");fullDepth.value="all";fullDepth.textContent="Caminho completo";folderDepthSelect.appendChild(fullDepth);');
    ...
    Html.Add('const svg=document.getElementById("graph"),viewport=document.getElementById("viewport"),ns="http://www.w3.org/2000/svg";let mode="folders",selected=null,activeFolderGroup=null,positions=new Map(),width=800,height=600;');
    ...
    Html.Add('function folderGroup(folder){if(folder==="(raiz do projeto)"||folder==="(desconhecido)")return folder;const depth=document.getElementById("folderDepth").value;if(depth==="all")return folder;if(/^[a-z]:[\\/]/i.test(folder)||folder.startsWith("\\\\"))return "(bibliotecas externas)";return folder.split(/[\\/]+/).slice(0,Number(depth)).join("\\")}function filesInGroup(group){return files.filter(x=>folderGroup(x.folder)===group)}');
    Html.Add('function filteredEdges(){const section=document.getElementById("section").value,term=document.getElementById("search").value.toLowerCase();return edges.filter(e=>(section==="all"||e.section===section)&&(!activeFolderGroup||folderGroup(folderOf(e.from))===activeFolderGroup||folderGroup(folderOf(e.to))===activeFolderGroup)&&(!term||`${e.consumer} ${e.used} ${e.from} ${e.to} ${folderOf(e.from)} ${folderOf(e.to)}`.toLowerCase().includes(term)))}');
    ...
    Html.Add('const names=new Set([folderGroup(folderOf(entryKey))]);for(const e of active){names.add(folderGroup(folderOf(e.from)));names.add(folderGroup(folderOf(e.to)))}const items=[...names].map(folder=>{const group=filesInGroup(folder),physical=new Set(group.map(x=>x.folder));');
    Html.Add('return{key:folder,label:folder,caption:`${group.length} arquivo(s) • ${physical.size} pasta(s)`,external:group.some(x=>!["project","project-reference"].includes(x.origin)),detail:`${folder}\n${physical.size} pasta(s)\n${group.length} arquivo(s)\n\n${group.map(x=>x.name+"\n"+x.path).join("\n\n")}`}});draw(items,[...map.values()],folderGroup(folderOf(entryKey)),true)}');
    ...
    Html.Add('function setMode(value){mode=value;document.getElementById("folderMode").classList.toggle("active",mode==="folders");document.getElementById("unitMode").classList.toggle("active",mode==="units");selected=null;render()}function openFolder(groupName){const group=filesInGroup(groupName);activeFolderGroup=groupName;document.getElementById("search").value="";document.getElementById("detail").textContent=`${groupName}\n${new Set(group.map(x=>x.folder)).size} pasta(s)\n${group.length} arquivo(s)\n\n${group.map(x=>x.name+"\n"+x.path).join("\n\n")}`;setMode("units")}function openPhysicalFolder(folder){activeFolderGroup=null;const group=folders.get(folder)||[];document.getElementById("search").value=folder;document.getElementById("detail").textContent=`${folder}\n${group.length} arquivo(s)\n\n${group.map(x=>x.name+"\n"+x.path).join("\n\n")}`;setMode("units")}document.getElementById("folderMode").onclick=()=>{activeFolderGroup=null;document.getElementById("search").value="";setMode("folders")};document.getElementById("unitMode").onclick=()=>setMode("units");document.getElementById("search").oninput=()=>{activeFolderGroup=null;render()};document.getElementById("section").onchange=render;document.getElementById("folderDepth").onchange=()=>{activeFolderGroup=null;document.getElementById("search").value="";setMode("folders")};document.getElementById("fit").onclick=()=>{viewport.scrollLeft=0;viewport.scrollTop=0;svg.style.width="100%";svg.style.height="100%"};');
    Html.Add('const summaryBox=document.getElementById("folderSummary"),summaryList=summaryBox.children[1];summaryBox.children[0].textContent=`Resumo: ${folders.size} pastas únicas, ${files.length} arquivos`;for(const [folder,group]of [...folders].sort((a,b)=>a[0].localeCompare(b[0]))){const item=document.createElement("li"),outgoing=[...folderEdges.values()].filter(x=>x.from===folder).reduce((n,x)=>n+x.count,0),incomingCount=[...folderEdges.values()].filter(x=>x.to===folder).reduce((n,x)=>n+x.count,0);item.textContent=`${folder} — ${group.length} arquivo(s), ${incomingCount} entrada(s), ${outgoing} saída(s)`;summaryList.appendChild(item)}');
    Html.Add('const panel=document.getElementById("folders");for(const [folder,group]of [...folders].sort((a,b)=>a[0].localeCompare(b[0]))){const details=document.createElement("details"),summary=document.createElement("summary");summary.textContent=`${folder} (${group.length})`;summary.onclick=()=>openPhysicalFolder(folder);details.appendChild(summary);for(const f of group.sort((a,b)=>a.name.localeCompare(b.name))){const b=document.createElement("button");b.textContent=f.name;b.title=f.path;b.onclick=()=>{activeFolderGroup=null;document.getElementById("search").value=f.path;setMode("units");document.getElementById("detail").textContent=`${f.name}\n${f.path}\n\nPasta: ${f.folder}\nOrigem: ${f.origin}`};details.appendChild(b)}panel.appendChild(details)}');
