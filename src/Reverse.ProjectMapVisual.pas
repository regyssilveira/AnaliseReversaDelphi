...
begin
  ...
  try
    ...
    Html.Add('<button id="folderBack" disabled>Voltar um nível</button>');
    ...
    Html.Add('function setMode(value){mode=value;document.getElementById("folderMode").classList.toggle("active",mode==="folders");document.getElementById("unitMode").classList.toggle("active",mode==="units");selected=null;render()}function groupDetail(name,group){document.getElementById("detail").textContent=`${name}\n${new Set(group.map(x=>x.folder)).size} pasta(s)\n${group.length} arquivo(s)\n\n${group.map(x=>x.name+"\n"+x.path).join("\n\n")}`}');
    Html.Add('function openFolder(groupName){const group=filesInGroup(groupName),physical=new Set(group.map(x=>x.folder)),depth=document.getElementById("folderDepth").value;groupDetail(groupName,group);document.getElementById("search").value="";if(physical.size>1&&depth!=="all"&&Number(depth)<maxFolderDepth){folderHistory.push({folders:activeFolders?[...activeFolders]:null,depth});activeFolders=physical;document.getElementById("folderDepth").value=String(Number(depth)+1);setMode("folders")}else{activeFolders=physical;setMode("units")}}');
    Html.Add('function openPhysicalFolder(folder){activeFolders=new Set([folder]);folderHistory=[];const group=folders.get(folder)||[];document.getElementById("search").value="";groupDetail(folder,group);setMode("units")}function resetFolders(){activeFolders=null;folderHistory=[];document.getElementById("search").value="";setMode("folders")}');
    Html.Add('document.getElementById("folderBack").onclick=()=>{const previous=folderHistory.pop();if(!previous)return;activeFolders=previous.folders?new Set(previous.folders):null;document.getElementById("folderDepth").value=previous.depth;setMode("folders")};document.getElementById("folderMode").onclick=resetFolders;document.getElementById("unitMode").onclick=()=>{if(files.length>1000&&!activeFolders){document.getElementById("detail").textContent="Selecione primeiro uma pasta para evitar renderizar milhares de arquivos de uma vez.";return}setMode("units")};');
    Html.Add('document.getElementById("search").oninput=()=>{activeFolders=null;folderHistory=[];render()};document.getElementById("section").onchange=render;document.getElementById("folderDepth").onchange=()=>{activeFolders=null;folderHistory=[];document.getElementById("search").value="";setMode("folders")};document.getElementById("fit").onclick=()=>{viewport.scrollLeft=0;viewport.scrollTop=0;svg.style.width="100%";svg.style.height="100%"};');
