...
begin
  ...
  try
    ...
    Html.Add('<div class="switch"><button id="folderMode" class="active">Pastas</button><button id="unitMode">Units</button></div>');
    ...
    Html.Add('<button id="fit">Ajustar grafo</button><p id="status"></p><div id="folders"><strong>Pastas e arquivos</strong></div><div id="detail">Abra uma pasta na lista ou selecione um item do gráfico.</div>');
    ...
    Html.Add('const svg=document.getElementById("graph"),viewport=document.getElementById("viewport"),ns="http://www.w3.org/2000/svg";let mode="folders",selected=null,positions=new Map(),width=800,height=600;');
    ...
    Html.Add('function filteredEdges(){const section=document.getElementById("section").value,term=document.getElementById("search").value.toLowerCase();return edges.filter(e=>(section==="all"||e.section===section)&&(!term||`${e.consumer} ${e.used} ${e.from} ${e.to} ${folderOf(e.from)} ${folderOf(e.to)}`.toLowerCase().includes(term)))}');
    Html.Add('function levels(root,nodes,links){const result=new Map([[root,0]]),q=[root];for(let i=0;i<q.length;i++)for(const e of links.filter(x=>x.from===q[i]))if(nodes.has(e.to)&&!result.has(e.to)){result.set(e.to,result.get(q[i])+1);q.push(e.to)}for(const n of nodes)if(!result.has(n))result.set(n,0);return result}');
    Html.Add('function draw(items,links,root,isFolder){svg.replaceChildren();const marker=el("marker",{id:"arrow",viewBox:"0 0 10 10",refX:9,refY:5,markerWidth:7,markerHeight:7,orient:"auto"},el("defs",{}));el("path",{d:"M 0 0 L 10 5 L 0 10 z",fill:"#59636e"},marker);const keys=new Set(items.map(x=>x.key)),lv=levels(root,keys,links),groups=new Map();for(const item of items){const level=lv.get(item.key);if(!groups.has(level))groups.set(level,[]);groups.get(level).push(item)}for(const group of groups.values())group.sort((a,b)=>a.label.localeCompare(b.label));const maxRows=Math.max(1,...[...groups.values()].map(x=>x.length)),maxLevel=Math.max(0,...groups.keys());width=(maxLevel+1)*280+80;height=maxRows*72+70;svg.setAttribute("viewBox",`0 0 ${width} ${height}`);svg.style.width=width+"px";svg.style.height=height+"px";positions=new Map();for(const[level,group]of groups)group.forEach((x,i)=>positions.set(x.key,{x:30+level*280,y:30+i*72}));');
    Html.Add('for(const e of links){const a=positions.get(e.from),b=positions.get(e.to);if(!a||!b)continue;const path=el("path",{d:`M ${a.x+220} ${a.y+22} C ${a.x+245} ${a.y+22},${b.x-25} ${b.y+22},${b.x} ${b.y+22}`,class:`edge ${e.section||""}${selected===e?" selected":""}`,"marker-end":"url(#arrow)"});path.addEventListener("click",()=>{selected=e;document.getElementById("detail").textContent=isFolder?`${e.from} → ${e.to}\n${e.count} relação(ões) entre as pastas`:`${e.consumer} usa ${e.used}\n${e.section}:${e.line}\n${e.source}`;render()});if(isFolder&&e.count>1)text((a.x+b.x+220)/2,(a.y+b.y+44)/2,String(e.count),svg,"edge-label")}');
    Html.Add('for(const item of items){const p=positions.get(item.key),classes=["node"];if(item.key===root)classes.push("entry");if(selected===item)classes.push("selected");if(item.external)classes.push("external");const g=el("g",{class:classes.join(" ")});el("rect",{x:p.x,y:p.y,width:220,height:44},g);text(p.x+10,p.y+19,item.label.length>31?item.label.slice(0,29)+"…":item.label,g);text(p.x+10,p.y+35,item.caption,g,"edge-label");g.addEventListener("click",()=>{selected=item;document.getElementById("detail").textContent=item.detail;render()})}document.getElementById("status").textContent=`${items.length} ${isFolder?"pasta(s)":"arquivo(s)"} | ${links.length} relação(ões)`}');
    Html.Add('function renderFolders(){const active=filteredEdges(),map=new Map();for(const e of active){const from=folderOf(e.from),to=folderOf(e.to);if(from===to)continue;const key=from+"\u0000"+to;if(!map.has(key))map.set(key,{from,to,count:0,edges:[]});const x=map.get(key);x.count++;x.edges.push(e)}const names=new Set([folderOf(entryKey)]);for(const e of active){names.add(folderOf(e.from));names.add(folderOf(e.to))}const items=[...names].map(folder=>{const group=folders.get(folder)||[];return{key:folder,label:folder,caption:`${group.length} arquivo(s)`,external:group.some(x=>!["project","project-reference"].includes(x.origin)),detail:`${folder}\n${group.length} arquivo(s)\n\n${group.map(x=>x.name+"\n"+x.path).join("\n\n")}`}});draw(items,[...map.values()],folderOf(entryKey),true)}');
    ...
    Html.Add('function setMode(value){mode=value;document.getElementById("folderMode").classList.toggle("active",mode==="folders");document.getElementById("unitMode").classList.toggle("active",mode==="units");selected=null;render()}document.getElementById("folderMode").onclick=()=>setMode("folders");document.getElementById("unitMode").onclick=()=>setMode("units");document.getElementById("search").oninput=render;document.getElementById("section").onchange=render;document.getElementById("fit").onclick=()=>{viewport.scrollLeft=0;viewport.scrollTop=0;svg.style.width="100%";svg.style.height="100%"};');
    Html.Add('const panel=document.getElementById("folders");for(const [folder,group]of [...folders].sort((a,b)=>a[0].localeCompare(b[0]))){const details=document.createElement("details"),summary=document.createElement("summary");summary.textContent=`${folder} (${group.length})`;summary.onclick=()=>{document.getElementById("search").value=folder;setMode("folders")};details.appendChild(summary);for(const f of group.sort((a,b)=>a.name.localeCompare(b.name))){const b=document.createElement("button");b.textContent=f.name;b.title=f.path;b.onclick=()=>{document.getElementById("search").value=f.path;setMode("units");document.getElementById("detail").textContent=`${f.name}\n${f.path}\n\nPasta: ${f.folder}\nOrigem: ${f.origin}`};details.appendChild(b)}panel.appendChild(details)}');
