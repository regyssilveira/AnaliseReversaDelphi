...
No modo padrão, sem `--unit`, a análise começa no DPR e inclui somente arquivos realmente alcançáveis por declarações `uses`, mesmo quando estão no Search Path ou em `--source-root`. O HTML abre na visão **Pastas**, agrega a quantidade de relações entre diretórios e permite alternar para **Units**. O controle **Agrupar diretórios** detecta a profundidade real da árvore e oferece todos os níveis encontrados — 1, 2, 3 e assim por diante — além do caminho completo. Clicar num grupo com subpastas entra no próximo nível; **Voltar um nível** retorna ao agrupamento anterior. Ao chegar numa pasta física, o clique abre apenas suas units, evitando tentar renderizar milhares de arquivos de uma vez. Os quadros exibem nomes mais longos e mostram o caminho completo ao apontar o mouse. A lateral apresenta um resumo sem duplicações, com arquivos e relações de entrada e saída por pasta física, seguido da árvore **Pastas e arquivos**. Selecionar uma unit muda para o grafo detalhado filtrado nesse arquivo. A busca aceita pasta, unit ou caminho completo, e o filtro separa DPR, `interface` e `implementation`.
...
Arquivos descobertos são identificados pela origem: pasta do projeto, referência explícita do projeto, Search Path do projeto, Search Path global da IDE ou `--source-root`. As cadeias recebem um destino estruturado: DPR, arquivo do projeto, raiz de biblioteca, sem consumidor conhecido ou ciclo. No HTML, **Mostrar usos fora do DPR** isola os ramos que continuam somente em bibliotecas ou arquivos desvinculados da entrada do projeto; o filtro **Origem dos arquivos** permite inspecionar cada grupo. **Mostrar todas as rotas** restaura o conjunto completo. O console e o `result.txt` apresentam a contagem de cada tipo e o relatório textual marca os finais com `[DPR]`, `[PROJECT]`, `[LIBRARY]`, `[NO CONSUMER]` ou `[CYCLE]`.
