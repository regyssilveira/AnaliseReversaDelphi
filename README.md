...
No modo padrão, sem `--unit`, a análise começa no DPR e inclui somente arquivos realmente alcançáveis por declarações `uses`, mesmo quando estão no Search Path ou em `--source-root`. O HTML abre na visão **Pastas**, agrega a quantidade de relações entre diretórios e permite alternar para **Units**. Clicar numa pasta do gráfico abre suas units; os quadros exibem nomes mais longos e mostram o caminho completo ao apontar o mouse. A lateral apresenta um resumo sem duplicações, com arquivos e relações de entrada e saída por pasta, seguido da árvore **Pastas e arquivos**. Selecionar uma unit muda para o grafo detalhado filtrado nesse arquivo. A busca aceita pasta, unit ou caminho completo, e o filtro separa DPR, `interface` e `implementation`.
