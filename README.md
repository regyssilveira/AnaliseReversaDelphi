...
No modo padrão, sem `--unit`, a análise começa no DPR e inclui somente arquivos realmente alcançáveis por declarações `uses`, mesmo quando estão no Search Path ou em `--source-root`. O HTML abre na visão **Pastas**, agrega a quantidade de relações entre diretórios e permite alternar para **Units**. A árvore lateral **Pastas e arquivos** pode ser aberta para listar todas as units de cada diretório; selecionar uma unit muda para o grafo detalhado filtrado nesse arquivo. A busca aceita pasta, unit ou caminho completo, e o filtro separa DPR, `interface` e `implementation`.
