...
param([Parameter(Mandatory)][ValidatePattern('^[A-Za-z0-9._-]+$')][string]$Version)

$ErrorActionPreference = 'Stop'
$workspaceRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$binaryPath = Join-Path $workspaceRoot 'bin\IDE\UnitBacktraceIDE.bpl'
if (-not (Test-Path -LiteralPath $binaryPath -PathType Leaf)) {
    throw "Package not found: $binaryPath. Run tools\Build-IDE.ps1 first."
}

$stagePath = Join-Path $workspaceRoot ('bin\package-ide-' + [guid]::NewGuid().ToString('N'))
...
foreach ($name in @('LICENSE','NOTICE','README.md','README.en.md','IDE.md','THIRD_PARTY_LICENSES.md')) {
...
$archivePath = Join-Path $workspaceRoot "bin\UnitBacktraceIDE-$Version-Delphi13.zip"
